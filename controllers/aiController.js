...
function buildFallbackSubjectGapStudyPlan({ subject, topic, studyGoal, currentLevel, timePerDay, classLevel }) {
  const subjectName = normalizeSubject(subject);
  const focusTopic = String(topic || "Core Concepts").trim() || "Core Concepts";
  const planTopics = [focusTopic, "Previous Basics", "Practice Questions", "Mistake Review", "Final Revision"];
  const minutes = String(timePerDay || "").includes("2") ? 35 : String(timePerDay || "").includes("3") ? 45 : 25;
  const weeks = planTopics.map((item, index) => ({
    week: `Week ${index + 1}`,
    title: item,
    status: index === 0 ? "active" : "locked",
    progress: index === 0 ? 25 : 0,
    focus: index === 0 ? "Build basics and detect current gaps." : "Unlocks after previous week progress.",
    tasks: [
      {
        id: `week-${index + 1}-read`,
        type: "Read",
        title: `Read: ${item}`,
  ...
  return {
    ...
    currentLevel,
    timePerDay,
    summary: `Start with ${focusTopic}, complete bite-sized daily tasks, then use quiz scores to update the next focus area.`,
    overallProgress: 25,
    weeks,
    dailyTasks: weeks[0].tasks,
    adaptiveRules: [
      "If quiz score is below 60%, repeat the weak topic with easier practice.",
      "If quiz score is 60-79%, add mixed questions and one revision task.",
      "If quiz score is 80% or above, unlock the next topic."
    ]
  };
}
...
exports.generateHighSchoolSubjectGapPlan = asyncHandler(async (req, res) => {
  const profile = await StudentProfile.findOne({ userId: req.user.id })
  ...
  const topic = String(req.body?.topic || "Core Concepts").trim().slice(0, 80);
  ...
  let plan = buildFallbackSubjectGapStudyPlan({ subject, topic, studyGoal, currentLevel, timePerDay, classLevel });
  let source = "fallback";
  let provider = "local";
  let model = "deterministic";
  ...
  try {
    const prompt = [
      ...
      "Rules: subject-based roadmap, weekly structure, bite-sized daily tasks, quiz/practice included, mobile-friendly text, no random data."
    ].join("\n");
    ...
    const ai = await requestAiResponse({
      ...
      context: {
        assistantMode: "general",
        feature: "highschool_subject_gap_plan",
        expectedFormat: "json",
        learnerStage: "highschool"
      }
    });
    ...
    if (parsed?.summary && Array.isArray(parsed?.weeks) && parsed.weeks.length) {
      const weeks = parsed.weeks
        .map((week, index) => ({
          ...
          title: String(week?.title || (index === 0 ? topic : "Next Topic")).trim().slice(0, 80),
          ...
          focus: String(week?.focus || "Complete daily tasks and quiz practice.").trim().slice(0, 160),
          tasks: Array.isArray(week?.tasks)
            ? week.tasks.map((task, taskIndex) => ({
                id: String(task?.id || `week-${index + 1}-task-${taskIndex + 1}`).trim().slice(0, 80),
      ...
      plan = {
        ...
        dailyTasks: Array.isArray(parsed.dailyTasks)
          ? parsed.dailyTasks.map((task, index) => ({
              id: String(task?.id || `daily-task-${index + 1}`).trim().slice(0, 80),
              type: String(task?.type || "Practice").trim().slice(0, 20),
              title: String(task?.title || "Practice: 10 Questions").trim().slice(0, 100),
              duration: String(task?.duration || "15 min").trim().slice(0, 30),
              completed: Boolean(task?.completed)
        ...
        adaptiveRules: Array.isArray(parsed.adaptiveRules)
          ? parsed.adaptiveRules.map((item) => String(item || "").trim()).filter(Boolean).slice(0, 4)
          : plan.adaptiveRules
      };
      source = "ai";
      provider = ai.provider;
